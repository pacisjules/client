...
function View_ProductsRecord() {
  ...
  // Ajax Start!
  $.ajax({
    url: `functions/menu/getallproductsbyspttablet.php?company=${company_ID}&spt=${sales_point_id}`,
    method: "POST",
    context: document.body,
    success: function (response) {
      if (response) {
...
function AddToCartTablet(id,namepro,realprice, qty) {
   if (!id || !realprice  || !namepro || !qty) {
       console.log("Product information is incomplete. Cannot add to cart.");
       return;
   }
...
function addCartTablet(id, namepro,p_price, p_benefit) {
...
if (isNaN(qty) || qty <= 0) {
    ...
    // If all checks pass, proceed with adding to cart
    AddToCartTablet(id,namepro,realprice, qty);
...
function proceed_tablet_sales () {
  ...
  // // Start AJAX request
  $.ajax({
    url: "functions/sales/bulksalesresto.php",
    method: "POST",
    dataType: 'json',
    data: {
      product_id: productIds,
