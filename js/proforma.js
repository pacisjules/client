...
function View_AllProformaRecord() {
  ...
  $.ajax({
    url: `functions/proforma/getallProformadata.php?spt=${sales_point_id}`,
    method: "POST",
    context: document.body,
    success: function (response) {
      if (response) {
...
function printInvoiceFunc() {
  ...
  $.ajax({
      url: `functions/proforma/printInvoice.php?sess_id=${ssess}`,
      method: 'GET',
      dataType: 'json',
      success: function(data) {
          if (data && data.data && data.data.length > 0) {
              ...
              const typereport = " Proforma Receipt";
              
              printInvoice(salesdata, typereport, sumtotal,date,customer_name,phone);
          } else {
...
function printInvoice(salesdata,typereport,sumtotal,date,customer_name,phone) {
