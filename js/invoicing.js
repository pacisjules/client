...
$('#clearItemBtn').click(function () {
 ...
 var company_ID = localStorage.getItem("CoID");
  ...
  $("#searcProductingNow").on("input", function (e) {
    ...
    // Ajax Start!
    $.ajax({
      url: `functions/request/getrequestbyPlaque.php?spt=${sales_point_id}&platename=${e.target.value}`,
      method: "POST",
      context: document.body,
      success: function (response) {
        if (response) {
          //console.log(response);
          $("#getseachrequest").html(response);
        } else {
          //console.log(response);
          $("#getseachrequest").html("Not Any result");
        }
      },
      error: function (xhr, status, error) {
        // console.log("AJAX request failed!");
...
function getSelected(id,name,price,C_Qty) {
  console.log(name);
...
function getRequestSelected(id,carName,platename,names,phone,carType,servicesNeeded,status) {
  console.log(names);
  
  $("#gettedcar").html(carName);
  $("#gettedplate").html(platename);
  $("#gettedtype").html(carType);
  $("#gettedname").html(names);
  $("#gettedphone").html(phone);
  $("#gettedservice").html(servicesNeeded);
  


  $("#getseachrequest").html('');
