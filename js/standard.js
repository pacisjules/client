    ...
    $.ajax({
      ...
      success: function(data) {
        ...
        $.each(data.data, function(index, item) {
          ...
          html += `<td class="d-flex flex-row justify-content-start align-items-center"><button class="btn btn-success getEditSales" type="button" data-bs-target="#edit_stand_modal" data-bs-toggle="modal" onclick="getupdatestandID('${item.id}','${item.raw_material_name}','${item.quantity}')"><i class="fa fa-edit" style="color: rgb(255,255,255);"></i></button><button class="btn btn-danger getremoveSales" type="button" style="margin-left: 20px;" data-bs-target="#delete_stand_modal" data-bs-toggle="modal" onclick="getStandIDremove('${item.id}','${item.raw_material_name}')" "><i class="fa fa-trash"></i></button></td>`; // Replace with your action buttons
          html += '</tr>';
        });
      ...
      error: function() {
        console.log('An error occurred while fetching standard details.');
      }
    });
...
$('#clearItemBtn').click(function () {
 ...
 $("#savep_sell").click(function () {
  ...
  // // Start AJAX request
  $.ajax({
    url: "functions/standard/setProductionStandard.php",
    method: "POST",
    dataType: 'json',
    data: {
      row_id: rowIds,
...
$("#updatestandard").click(function () {
  ...
  //Ajax Start!
  $.ajax({
    url: "functions/standard/updateStandard.php",
    method: "POST",
...
$("#updaterowmaterial").click(function () {
  ...
  var set_qty = parseFloat($("#row_qty").val());
  var set_id = parseInt(localStorage.getItem("row_id"));

  //Ajax Start!
  $.ajax({
    url: "functions/standard/updateStandardrow.php",
    method: "POST",
...
$("#removeStandard").click(function () {
  $("#removeStandard").html("Please wait..");
  var set_id = parseInt(localStorage.getItem("set_id"));

  //Ajax Start!
  $.ajax({
    url: "functions/standard/deletestandard.php",
    method: "POST",
...
var company_ID = localStorage.getItem("CoID");
  ...
  $.ajax({
    url: `functions/standard/getAllSettedStandard.php?company_ID=${company_ID}`,
    method: "POST",
    context: document.body,
    success: function (response) {
      if (response) {
        ...
        //console.log(response);
        $("#standard_records").html("Not Any result");
      }
    },
    error: function (xhr, status, error) {
      console.log("AJAX request failed!");
...
//   };
  ...
  $.ajax({
    url: `functions/standard/getlast5settedStandard.php?company_ID=${company_ID}`,
    method: "POST",
    context: document.body,
    success: function (response) {
      if (response) {
        //console.log(response);
        $("#lastsetted_table").html(response);
        
      } else {
        
        $("#lastsetted_table").html("Not Any result");
      }
    },
    error: function (xhr, status, error) {
      console.log("AJAX request failed!");
...
function RemoveProductID(id,name){
  console.log(id);
...
function getupdatestandID(id,row,qty){
  console.log(id);
  console.log(row);
...
function getStandIDremove(id,name){
  console.log(id);
  $("#row_name").html(name)
  localStorage.setItem("row_id",id);
}
