$(document).ready(function () {
  ...
  $.ajax({
    url: `functions/multistore/getTransferProductDetails.php?store_id=${store_id}&product_id=${product_id}`,
    method: 'GET',
    success: function(data) {
      ...
      $.each(data.data, function(index, item) {
        ...
        html += '<td>'+num+'. ' + item.name + '</td>';
         html += '<td> ' + item.unit_id + '</td>';
          html += '<td>' + item.box_or_carton + '</td>';
        html += '<td>' + item.quantity_per_box + '</td>';
        html += '<td>' + item.username + '</td>';
        html += '<td>' + item.created_at + '</td>';
        html += `<td class="d-flex flex-row justify-content-start align-items-center"><button class="btn btn-success getEditSales" type="button" data-bs-target="#edit_sales_modal" data-bs-toggle="modal" onclick="getSalesID('${item.id}','${item.product_id}','${item.created_at}','${item.box_or_carton}','${item.quantity_per_box}','${store_id}')"><i class="fa fa-edit" style="color: rgb(255,255,255);"></i></button><button class="btn btn-danger getremoveSales" type="button" style="margin-left: 20px;" data-bs-target="#delete_sales_modal" data-bs-toggle="modal" onclick="getSalesIDremove('${item.id}','${item.product_id}','${store_id}')" "><i class="fa fa-trash"></i></button>
         </td> `; 
        html += '</tr>';
      });
      $('#transferdetail_table').html(html);
...
// pick date for editing
 ...
 $("#editBtnTransfer").on("click", function() {
        ...
        var trans = localStorage.getItem("trans_id");
        var product_id = localStorage.getItem("prod_id");
        ...
        $.ajax({
            type: "POST",
            url: "functions/multistore/updateTransferProd.php", // Update this with the actual path to your PHP script
            data: {
                trans_id: trans,
...
$("#deleteBtntrans").on("click", function() {
    ...
    $.ajax({
        type: "POST",
        url: "functions/multistore/deletetransfer.php", // Update this with the actual path to your PHP script
        data: {
            trans_id: trans_id, // Corrected variable name to match the PHP script
            product_id: product_id,
            store_id: store_id, // Corrected variable name to match the PHP script
...
// }
  ...
  function SelectDeleteInventory(e, name) {
   ...
   function getSalesID(id,product_id,created_at,box,item_per_box,store_id){
     localStorage.setItem("trans_id", id);
        localStorage.setItem("prod_id", product_id);
        localStorage.setItem("created_at", created_at);
        localStorage.setItem("store_id", store_id);
        $("#editquantity").val(item_per_box);
        $("#editbox").val(box);
        
        console.log("trans ", id);
        console.log("product_id ", product_id);
        console.log("created_at ", created_at);
     console.log("item_per_box ", item_per_box);
        console.log("box ", box);
         console.log("store_id ", store_id);
}

function getSalesIDremove(id,product_id,store_id){
   localStorage.setItem("trans_id", id);
