$(document).ready(function () {
    ...
    var trainer_id = getParameterByName('trainer_id'); 
    ...
    $.ajax({
      url: `functions/departements/getallCoursesOfTrainer.php?dep_id=${dep_id}&trainer_id=${trainer_id}`,
      method: 'GET',
      success: function(data) {
        // Handle the data received from the AJAX request and display it in the table
        var html = '';

        var trainername = data.data[0].full_name;
        ...
        $.each(data.data, function(index, item) {
          ...
          html += '<td>' + item.departement + '</td>';

          html += `<td class="d-flex flex-row justify-content-start align-items-center">
          
          <button class="btn btn-danger getremoveSales" type="button" style="margin-left: 20px;" data-bs-target="#delete_sales_modal" data-bs-toggle="modal" onclick="getSalesIDremove('${item.course_id}','${item.trainer_id}')" "><i class="fa fa-trash"></i> Remove</button>
           </td> `; 
          html += '</tr>';
        });
        $('#trainercourse_table').html(html);
