$(document).ready(function () {
  ...
  $("#saveproduct").click(function () {
    ...
    if ($("#ExpCheck").is(":checked")) {
      allow_expiration = 1;
      var time_addons = $("#time_addons").val();
    } else {
      allow_expiration = 0;
      var time_addons = 0;
    }
    ...
    // Validate all values
    if (!name || !price || !benefit || !category_id) {
      $("#save_message").html("Please fill in all fields");
      return;
    }

    if (isNaN(price) || isNaN(benefit) || isNaN(barcode)) {
      $("#save_message").html("Price, benefit and barcode must be numbers");
      return;
    }

    if (price <= 0 || benefit <= 0) {
      $("#save_message").html(
        "Price, benefit and barcode must be greater than 0"
      );
      return;
    }

    if (category_id === "") {
      $("#save_message").html("Please select a category");
      return;
    }
...
function RemoveProductID(e) {
  console.log("Selected Product id: ", e);
...
function printProducthistory(historydata, typereport) {
  ...
  for (let i = 0; i < historydata.length; i++) {
    ...
    table += `<tr >
    <td style="font-size: 12px;font-family: 'Open Sans', sans-serif; color: #1e2b33; font-weight: normal;  vertical-align: top; padding: 0 0 7px;" align="center" width="150">${
      i + 1
    }. ${item.full_name}</td>
    <td style="font-size: 12px;font-family: 'Open Sans', sans-serif; color: #1e2b33; font-weight: normal;  vertical-align: top; padding: 0 0 7px;" align="center" width="150">${
      item.action
