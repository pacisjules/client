...
function View_DayRecorddetail(session_id) {
    ...
    var table = $('#employeeTable').DataTable({
        destroy: true,  // Allow reinitializing the table
        "ajax": {
            "url": `functions/transaction/getpaymentsession.php?session_id=${session_id}&spt=${spt}`,
            "dataSrc": function(json) {
                if (json.data && Array.isArray(json.data)) {
                    // Populate sumtotal at the bottom
                    $('#sum_total').html(`${Intl.NumberFormat('en-US').format(json.sumtotal)} Rwf`);
        ...
        "columns": [
            ...
            { 
                "data": "PaymentMethod",
                "render": function(data, type, row, meta) {
                    return `<p style="text-transform:uppercase; font-weight: bold;">${row.PaymentMethod}</p>`;
                }
            },
            { "data": "Amount" },
            { 
                ...
                "render": function(data, type, row, meta) {
                    ...
                    return `
                       <td class="d-flex justify-content-start align-items-center">
                        <button class="btn btn-success getEditSales me-2" type="button" data-bs-target="#edit_payment_modal" data-bs-toggle="modal" onclick="getPaymentID('${row.PaymentID}','${row.PaymentMethod}','${row.Amount}','${row.PaymentDate}')">
                            <i class="fa fa-edit" style="color: rgb(255,255,255);"></i>
                        </button>
                        <button class="btn btn-danger getremoveSales" type="button" data-bs-target="#delete_payment_modal" data-bs-toggle="modal" onclick="getdeleteID('${row.PaymentID}')">
        ...
        "initComplete": function(settings, json) {
            ...
            var jobTitles = [];
            json.data.forEach(function(employee) {
                if (!jobTitles.includes(employee.PaymentMethod)) {
                    jobTitles.push(employee.PaymentMethod);
                }
            });
            jobTitles.sort();
            jobTitles.forEach(function(title) {
