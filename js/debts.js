$(document).ready(function () {
  ...
  $("#getdailyDebtsall").click(function () {
    DailyCustomerDebtReport();
});

$("#getdailypaidsall").click(function () {
getallCustomerPaidPrint();
});

$("#getMonthlyDebtsall").click(function () {
...
const startDate = new Date(year, month - 1, 1); // Subtract 1 from month to make it zero-based
 ...
 endDate.setHours(23, 59, 59, 999); // Set time to end of the day
  ...
  MonthlyCustomerDebtReport(formattedStartDate,formattedEndDate);
});



$("#getMonthlyPaidsall").click(function () {
  const selectedMonth = $("#monthSelectnew").val();
...
const startDate = new Date(year, month - 1, 1); // Subtract 1 from month to make it zero-based
 ...
 endDate.setHours(23, 59, 59, 999); // Set time to end of the day
  ...
  const formattedStartDate = startDate.toISOString().slice(0, 10);
  const formattedEndDate = endDate.toISOString().slice(0, 10);
  MonthlyCustomerPaidReport(formattedStartDate,formattedEndDate);
});
  ...
  function DailyCustomerDebtReport(){
  ...
  const formatDate = (myDate) => {
    ...
    // Make an AJAX request to fetch data by customer_id
    $.ajax({
      url: `functions/debts/getdailyCompanyDebts.php?spt=${sales_point_id}&date=${formattedDate}`,
      method: 'GET',
      success: function(response) {
      console.log(response.data);
            const debtdata = response.data;
            const total_debtcust = response.total_debtcust;
            const typereport = " Daily  Debts Report";
            printDailyCustomerDebts(debtdata,typereport,total_debtcust);
...
function MonthlyCustomerDebtReport(startDate, endDate){
    // Retrieve the customer_id from the query parameter
        ...
        // Make an AJAX request to fetch data by customer_id
        $.ajax({
          url: `functions/debts/getMonthlyCompanyDebts.php?spt=${sales_point_id}&startdate=${startDate}&enddate=${endDate}`,
          method: 'GET',
          success: function(response) {
          console.log(response.data);
                const debtdata = response.data;
                const total_debtcust = response.total_debtcust;
                const typereport = " Monthly  Debts Report";
                printDailyCustomerDebts(debtdata,typereport,total_debtcust);
...
function MonthlyCustomerPaidReport(startDate, endDate){
          // Retrieve the customer_id from the query parameter
              ...
              // Make an AJAX request to fetch data by customer_id
              $.ajax({
                url: `functions/debts/getMonthlyCompanyPaid.php?spt=${sales_point_id}&startdate=${startDate}&enddate=${endDate}`,
                method: 'GET',
                success: function(response) {
                console.log(response.data);
                const debstdata = response.debts;
                const totalamount = response.total_debt;
                const typereport = "Monthly Paid Debts Report";
    ...
    $.ajax({
      ...
      success: function (response) {
        if (response) {
          ...
          printAllDebtsOnlyReport(debstdata,totalamount,typereport);
  ...
  function populateMonthDropdown() {
   ...
   function getallCustomerPaidPrint() {
  ...
  const formatDate = (myDate) => {
    ...
    $.ajax({
      url:`functions/debts/getalldebtscompanysptPaid.php?spt=${sales_point_id}&date=${formattedDate}`,
      method: "POST",
      context: document.body,
      success: function (response) {
        if (response) {
          ...
          const debstdata = response.debts;
          const totalamount = response.total_debt;
          const typereport = "Daily Paid Debts Report";
          printAllDebtsReport(debstdata,totalamount,typereport);
...
function  printDailyCustomerDebts(debtdata,typereport,total_debtcust) {
  // Calculate the total amount with interest
  const currentDate = new Date();
...
for (let i = 0; i < debtdata.length; i++) {
  ...
  <td style="font-size: 12px;font-family: 'Open Sans', sans-serif; color: #1e2b33; font-weight: normal;  vertical-align: top; padding: 0 0 7px;" align="center" width="150">${item.names}</td>
        ...
        <td>
          ...
          <table width="220" border="0" cellpadding="0" cellspacing="0" align="right" class="col">
            <tbody>
              <tr>
                <td style="font-size: 16px; color: #1f0c57; font-family: 'Open Sans', sans-serif; line-height: 18px; vertical-align: top; text-align: right;">
                  <strong>Total Daily Amount: ${new Intl.NumberFormat("en-US", {
              style: "currency",
              currency: "RWF",
          }).format(parseFloat(total_debtcust))}</strong>
                </td>
    ...
    <td>
      ...
      <table width="600" border="0" cellpadding="0" cellspacing="0" align="center" class="fullPadding">
        ...
        <tr>
          ...
          <td style="font-size: 14px; font-family: 'Open Sans', sans-serif; color: #000; line-height: 22px; vertical-align: top; text-align:right; ">
             <strong>Total Amount Paid: ${new Intl.NumberFormat("en-US", {
              style: "currency",
              currency: "RWF",
...
function printAllDebtsOnlyReport(debstdata,totalamount,typereport) {
  // Calculate the total amount with interest
  const currentDate = new Date();
...
function populateMonthDropdownnew() {
  // Populate the month dropdown with options (e.g., from January to December)
  const monthSelect = $("#monthSelectnew");
  const currentDate = new Date();
  const currentYear = currentDate.getFullYear();
