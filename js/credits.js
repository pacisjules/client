$(document).ready(function () {
  ...
  $("#getcustomerhistory").click(function () {
      View_customerpaymnetPrint(supplier_id);
  });
  
   $("#getcustomerdebts").click(function () {
      CustomerDebtReport(supplier_id);
  });
  ...
  $.ajax({
    url: `functions/credits/getalldebtsbycustomer.php?spt=${sales_point_id}&supplier_id=${supplier_id}`,
    method: 'GET',
  ...
  $("#InsertDebts").click(function () {
    ...
    // Create a new Date object with the selected date
        ...
        $.ajax({
            type: "POST",
            url: "functions/credits/insertdebt.php", // Replace with the actual URL of your PHP script
            data: {
  ...
  var amountPaid = localStorage.getItem("debt_amount_paid");
      ...
      $.ajax({
          type: "POST",
          url: "functions/credits/payOneItemdbts.php", // Replace with the actual URL of your PHP script
          data: {
              id: debt_id,
              qty: Quantity,
              amount: amountDue,
  ...
  $("#RemoveDebts").click(function () {
    ...
    // Add an input field for descriptions if not present
        var sales_point_id = localStorage.getItem("SptID");
        ...
        $.ajax({
            type: "POST",
            url: "functions/credits/payIntranche.php", // Replace with the actual URL of your PHP script
            data: {
                id: supplier_id,
                amount: amount,
                spt: sales_point_id,
   ...
   $("#payalldebt").click(function () {
       ...
       var supplier_id = localStorage.getItem("supplier_id");
        ...
        $.ajax({
            type: "POST",
            url: "functions/credits/payalldebtofcustomer.php", // Replace with the actual URL of your PHP script
            data: {
                sess_id: supplier_id,
                status: status,
  ...
  function CustomerDebtReport(supplier_id){
  ...
  // Make an AJAX request to fetch data by customer_id
  $.ajax({
    url: `functions/credits/getalldebtsbycustomerPrint.php?spt=${sales_point_id}&sess_id=${supplier_id}`,
    method: 'GET',
    success: function(response) {
    console.log(response.data);
          ...
          const typereport = " Supplier Debts Report";
   ...
   function View_customerpaymnetPrint(supplier_id) {
    ...
    $.ajax({
      url:`functions/credits/paymenthistory.php?supplier_id=${supplier_id}&spt=${sales_point_id}`,
      method: "POST",
      context: document.body,
      success: function (response) {
        if (response) {
          ...
          const total_paidcust = response.total_paidcust;
          const total_balance = response.total_balance;
          const typereport = " Credits Payment History Report";
  ...
  function View_allCustomerDebtsPrint() {
    ...
    $.ajax({
      url:`functions/credits/getalldebtscompanysptNotPaid.php?spt=${sales_point_id}`,
      method: "POST",
      context: document.body,
      success: function (response) {
        if (response) {
          ...
          const typereport = "My All Credits Report";
  ...
  function View_customerDebtsRecord() {
    ...
    // Ajax Start!
    $.ajax({
        url: `functions/credits/getalldebtscompanyspt.php?spt=${sales_point_id}`,
            ...
            <tbody>
              ...
              <tr>
                <td style="font-size: 16px; color: #1f0c57; font-family: 'Open Sans', sans-serif; line-height: 18px; vertical-align: top; text-align: right;">
                  <strong>Supplier Names: ${names}</strong>
                </td>
              </tr>
    ...
    <td>
      ...
      <table width="600" border="0" cellpadding="0" cellspacing="0" align="center" class="fullPadding">
        ...
        <tr>
          ...
          <td style="font-size: 14px; font-family: 'Open Sans', sans-serif; color: #000; line-height: 22px; vertical-align: top; text-align:right; ">
             <strong>Total Credits Amount: ${new Intl.NumberFormat("en-US", {
              style: "currency",
              currency: "RWF",
...
function populateSuppier() {
    var sales_point_id = parseInt(localStorage.getItem("SptID"));

    $.ajax({
        url: "functions/credits/getSupplierForCredit.php",
