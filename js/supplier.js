$(document).ready(function () {
    ...
    $("#addsupplier").click(function () {
      ...
      // Start AJAX request
      $.ajax({
        url: "functions/supplier/addnewsupplier.php",
        method: "POST",
        data: {
          names: names,
          phone: phone,
      ...
      $("#addsupplier").html("Please wait.."); // Update another element's text (saveNewUser)
    });
    ...
    $("#Editsupplier").click(function () {
      ...
      var company_ID = parseInt(localStorage.getItem("CoID"));
      var supplier_id = parseInt(localStorage.getItem("cust_id"));
  
      //Ajax Start!
      $.ajax({
        url: "functions/supplier/updatesupplier.php",
        method: "POST",
        ...
        success: function (response) {
          View_supplierRecord(); 
          $("#Editsupplier").html("Edit supplier");
          $("#modal_inventory").modal("hide");
          localStorage.removeItem("cust_id");
        },
        error: function (error) {
          $("#Editsupplier").html("Update");
          console.log(error.responseText);
              ...
              $("#removesupplier").click(function () {
                  $("#removesupplier").html("Please wait..");
                  ...
                  //Ajax Start!
                  $.ajax({
                  url: "functions/supplier/deletesupplier.php",
                  method: "POST",
                  ...
                  success: function (response) {
                      ...
                      View_supplierRecord(); 
                      $("#removesupplier").html("Delete");
                      $("#delete-modal").modal("hide");
                      $("#diagMsg").html("supplier removed");
                  },
                  error: function (error) {
                      console.log(error);
                      $("#removesupplier").html("Delete");
                ...
                $("#searchsupplier").on("input", function (e) {
                  ...
                  // Ajax Start!
                  $.ajax({
                    url: `functions/supplier/searchsupplierbyname.php?company=${company_ID}&name=${e.target.value}`,
                          ...
                          var html = `
                            <tr>
                              ...
                              <td class="d-flex flex-row justify-content-start align-items-center">
                                <button class="btn btn-success" type="button" data-bs-target="#modal_inventory" data-bs-toggle="modal" onclick="SelectEditsupplier('${row.supplier_id}', '${row.names}', '${row.phone}', '${row.address}')">
                                  <i class="fa fa-edit" style="color: rgb(255,255,255);"></i>
                                </button>
                               <button class="btn btn-danger" type="button" style="margin-left: 20px;" data-bs-target="#delete-modal" data-bs-toggle="modal" onclick="SelectDeletesupplier('${row.supplier_id}', '${row.names}')"><i class="fa fa-trash"></i></button>
                              </td>  
                            </tr>
                          `;
  ...
  function View_supplierRecord() {
    ...
    $.ajax({
      url:`functions/supplier/getsupplierbyspt.php?company=${company_ID}`,
      method: "POST",
      context: document.body,
      success: function (response) {
        if (response) {
...
function SelectEditsupplier(supplier_id, names, phone, address) {
    ...
    localStorage.setItem("cust_id", supplier_id);
}
  
  
  function SelectDeletesupplier(e, names) {
    console.log(e);
    $("#delnames").html(names);
    localStorage.setItem("cust_id", e);
    }
