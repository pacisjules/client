...
// Get sales point ID, start date (Monday), and end date (Sunday) from query string
$sales_point_id = $_GET['sales_point_id'];
$monday = $_GET['monday'];
$sunday = $_GET['sunday'];

// SQL query to fetch expenses data for the week
$sql = "
    SELECT
        SE.id,
        SE.name,
...
$stmt->bind_param("iss", $sales_point_id, $monday, $sunday); // "i" for integer and "s" for string
