...
//Set master path
$masterHome = "/functions/sales/getalldaysaleswithcompanysptweekname.php";
...
if ($_SERVER['REQUEST_METHOD'] === 'GET') {
    ...
    if ($_SERVER['REQUEST_URI'] === $masterHome . "/all_sales_days?company=" . $comID . "&spt=". $spt . "&monday=". $monday ."&sunday=" . $sunday ."&name=".$name) {
...
FROM
    ...
    inventory INV
WHERE 
SL.product_id=PD.id AND SL.product_id=INV.product_id AND SL.sales_point_id=SP.sales_point_id AND SP.company_ID=$comID AND SL.sales_point_id=$spt AND SL.created_time BETWEEN '$monday' AND '$sunday' AND PD.name LIKE '%$name%'
    ";
