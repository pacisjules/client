...
$sumQuery = "
SELECT 
    SUM(SL.total_amount) AS sumtotal,
    SUM(SL.total_benefit) AS sumbenefit
FROM
    sales SL
JOIN salespoint SP ON
    SL.sales_point_id = SP.sales_point_id
...
$sumtotal = (float)$sumData['sumtotal'];
$sumbenefit = (float)$sumData['sumbenefit'];
...
$sumTotalexpenses = "SELECT IFNULL(SUM(amount), 0.00) AS sumtotalexpenses FROM shop_expenses 
                        WHERE 
                        created_date >= '$start_date 00:00:00' 
