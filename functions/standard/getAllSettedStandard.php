...
while ($row = $result->fetch_assoc()) {
    ...
    $value .= '
        ...
        <td style="font-size: 12px;">'.$row['created_at'].'</td>
         ...
         <button class="btn btn-success" type="button" data-bs-target="#edit_product_modal" data-bs-toggle="modal" onclick="setUpdates(`'.$row['quantity'].'`, `'.$myid.'`)"><i class="fa fa-edit" style="color: rgb(255,255,255);"></i></button>
         <button class="btn btn-danger" type="button" style="margin-left: 20px;" data-bs-target="#delete-modal" data-bs-toggle="modal" onclick="RemoveProductID(`'.$myid.'`,`'.$row['Product_Name'].'`)"><i class="fa fa-trash"></i></button>
            ...
            <a style="font-size: 12px;margin-left:20px;" class="nav-link active" href="standarddetails.php?session_id=' . $prod_session . '&product_id='.$prod_id.'">
