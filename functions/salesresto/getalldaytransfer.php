...
// Fetch the company's timezone
$sqlcompany = "SELECT timezone_name FROM companies WHERE id=$comID";
$resultCompany = $conn->query($sqlcompany);
$rowCompany = $resultCompany->fetch_assoc();

$newdate = $rowCompany['timezone_name'];
$righttime = date('Y-m-d', strtotime($date));
...
while ($row = $result->fetch_assoc()) {
    ...
    $created_time = new DateTime($row['request_created_time']);
    $created_time->setTimezone(new DateTimeZone($newdate));
    $row['request_created_time'] = $created_time->format('Y-m-d H:i:s');
