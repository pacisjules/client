...
while ($row = $result->fetch_assoc()) {
    ...
    $value .= '
        <tr>
                                            ...
                                            <td class="d-flex flex-row justify-content-start align-items-center"><button class="btn btn-success" type="button" data-bs-target="#edit_product_modal" data-bs-toggle="modal" onclick="setUpdates(`'.$row['name'].'`,`'.$row['price'].'`, `'.$row['benefit'].'`, `'.$row['description'].'`, `'.$row['id'].'`)"><i class="fa fa-edit" style="color: rgb(255,255,255);"></i></button><button class="btn btn-danger" type="button" style="margin-left: 20px;" data-bs-target="#delete-modal" data-bs-toggle="modal" onclick="RemoveProductID(`'.$row['id'].'`)"><i class="fa fa-trash"></i></button><button class="'.$endis.'" type="button" style="margin-left: 20px;" data-bs-target="#disable-product" data-bs-toggle="modal" onclick="DisableProductID(`'.$row['id'].'`, `'.$row['status'].'`)"><i class="'.$icon.'"></i></button><button class="btn btn-primary" type="button" style="margin-left: 18px;min-width: 128px;" data-bs-target="#inventory" data-bs-toggle="modal" onclick="SetProductID(`'.$row['id'].'`)"=><svg xmlns="http://www.w3.org/2000/svg" width="1em" height="1em" fill="currentColor" viewBox="0 0 16 16" class="bi bi-house-fill">
                                                    ...
                                                    <button class="btn btn-primary" type="button" style="margin-left: 18px;background: rgb(223,139,78);font-weight: bold;border-color: rgb(255,255,255);width: 151.6875px;" data-bs-target="#sell-now" data-bs-toggle="modal" 
                                                    onclick="SetProductID(`'.$row['id'].'`,`'.$row['name'].'`,`'.$row['price'].'`,`'.$row['benefit'].'`, `'.$current_quantity.'`)"><svg xmlns="http://www.w3.org/2000/svg" width="1em" height="1em" fill="currentColor" viewBox="0 0 16 16" class="bi bi-cart-fill">
                                                        <path d="M0 1.5A.5.5 0 0 1 .5 1H2a.5.5 0 0 1 .485.379L2.89 3H14.5a.5.5 0 0 1 .491.592l-1.5 8A.5.5 0 0 1 13 12H4a.5.5 0 0 1-.491-.408L2.01 3.607 1.61 2H.5a.5.5 0 0 1-.5-.5zM5 12a2 2 0 1 0 0 4 2 2 0 0 0 0-4zm7 0a2 2 0 1 0 0 4 2 2 0 0 0 0-4zm-7 1a1 1 0 1 1 0 2 1 1 0 0 1 0-2zm7 0a1 1 0 1 1 0 2 1 1 0 0 1 0-2z"></path>
                                                    </svg>&nbsp; Sell it Now</button></td>
        </tr>
        ';
}
