...
$sql = "
    ...
    (SELECT COALESCE(SUM(transferHistory.box_or_carton * transferHistory.quantity_per_box), 0) AS entry FROM transferHistory WHERE transferHistory.product_id=products.id AND transferHistory.created_at LIKE '$date%' AND transferHistory.spt_id=$spt) AS entry_stock,
...
LEFT JOIN
    transferHistory ON products.id = transferHistory.product_id
WHERE 
