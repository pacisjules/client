...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $sql_query = "SELECT DT.id, DT.amount, DT.amount_paid,sum(DT.amount) as sum_amount, sum(DT.amount_paid) as sum_amount_paid, CT.names, CT.phone, CT.address FROM credits DT, supplier CT, salespoint SPT WHERE CT.supplier_id=DT.supplier_id AND SPT.sales_point_id=DT.sales_point_id AND DT.supplier_id =? AND DT.status = 1";
    ...
    if ($sql_result->num_rows > 0) {
            ...
            $update_sql = "update credits set  amount_paid=amount, status=3 where supplier_id=$sess_id";
