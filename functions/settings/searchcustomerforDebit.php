...
while ($row = $result->fetch_assoc()) {
     $num+=1;
    $value .= '
            <p style="margin-bottom: 0px; cursor:pointer;" class="hover-effect" onclick="getSelectedCustomer(`' . $row['customer_id'] . '`,`' . $row['names'] . '`,`' . $row['phone'] . '`,`' . $row['address'] . '`)">   ' . $num . '.  ' . $row['names'] . '</p>
            ';
