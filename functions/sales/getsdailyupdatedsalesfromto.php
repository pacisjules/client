...
// Escape inputs to avoid SQL injection (basic security measure)
$from = mysqli_real_escape_string($conn, $from);
$to = mysqli_real_escape_string($conn, $to);
$spt = (int)$spt; // Ensure spt is an integer
...
$sql = "
    SELECT
        ...
        (SELECT CONCAT(employee.first_name, ' ', employee.last_name) FROM employee WHERE employee.user_id = s.user_id) AS doneby
    FROM 
        sales s
    JOIN 
        customer c ON s.customer_id = c.customer_id
    WHERE 
        s.sales_point_id = $spt 
        AND s.created_time >= '$from' 
        AND s.created_time <= '$to'
    GROUP BY 
        s.sess_id
    ORDER BY 
        s.created_time DESC
